...
class DataGenerator:
    def __init__(self, seed: int = 42):
        ...
        self.base_time_ms = 1704067200000
...
def parse_args():
    ...
    parser.add_argument("--print-seed", action="store_true",
                        help="Print the chosen seed so random runs can be reproduced")
...
def choose_seed(seed: Optional[int], print_seed: bool) -> int:
    chosen_seed = seed if seed is not None else random.SystemRandom().randrange(0, 2 ** 32)
    if print_seed or seed is None:
        print(f"Generated seed: {chosen_seed}")
    return chosen_seed


def build_metadata(args: argparse.Namespace, seed: int, gen: DataGenerator) -> Dict[str, Any]:
    return {
        "seed": seed,
        "base_time_ms": gen.base_time_ms,
        "parameters": {
            "users": args.users,
            "orders": args.orders,
            "trades": args.trades,
        ...
        "note": "Use this seed and the same parameters to reproduce generated data byte-for-byte.",
    }


def main():
    args = parse_args()
    seed = choose_seed(args.seed, args.print_seed)
    ...
    # Export
    gen.export_json(os.path.join(args.output_dir, "metadata.json"), build_metadata(args, seed, gen))
